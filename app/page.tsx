        ...
        {!isAuthed ? (
          ...
          >
            <motion.div 
              ...
              className="mb-6"
            >
              <Lock className={`w-8 h-8 ${isWrong ? 'text-rose-500' : 'text-black'}`} strokeWidth={1.5} />
            </motion.div>

            <h2 className="text-black text-xl font-semibold mb-2">输入暗号</h2>
            <p className="text-gray-400 text-sm mb-8">请输入 4 位数字以继续</p>
            ...
            <div className="w-full max-w-[240px] relative">
              <input
                ...
                className={`
                  ...
                  bg-gray-50 border-2 rounded-2xl outline-none transition-all
                  ${isWrong ? 'border-rose-500 text-rose-500 shadow-[0_0_15px_rgba(244,63,94,0.2)]' : 'border-black focus:border-rose-400 text-black shadow-lg'}
                  ${isVerifying ? 'opacity-50' : 'opacity-100'}
                `}
                style={{ caretColor: '#fb7185' }} // 自定义光标颜色
              />
              ...
              <div className="flex gap-4 justify-center mt-6">
                {[0, 1, 2, 3].map((i) => (
                  <div 
                    key={i} 
                    className={`w-3 h-3 rounded-full border transition-colors ${password.length > i ? 'bg-black border-black' : 'bg-transparent border-gray-300'}`} 
                  />
                ))}
              </div>
            </div>

            <p className={`mt-8 text-[11px] tracking-[0.2em] font-bold ${isWrong ? 'text-rose-500' : 'text-gray-300'}`}>
              {isVerifying ? "正在校验..." : isWrong ? "暗号错误" : "IDENTITY ACCESS"}
            </p>
          </motion.div>
            ...
            <div className="relative z-20 text-center px-6 max-w-4xl pb-24">
              ...
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.8, duration: 1 }}>
                ...
                <div className="flex justify-center items-baseline gap-4 md:gap-8 mb-16">
                  ...
                  <motion.span animate={{ opacity: [1, 0.4, 1] }} transition={{ repeat: Infinity, duration: 1.5 }} className="font-mono text-rose-400/60 text-3xl md:text-5xl self-center mb-6">:</motion.span>
                  <div className="flex flex-col items-center">
                    <span className="font-mono text-white/90 text-4xl md:text-6xl w-[1.5em]">
